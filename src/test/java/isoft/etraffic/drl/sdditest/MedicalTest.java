...
import isoft.etraffic.drl.sddipages.MedicalTestPage;
...
public class MedicalTest {
	...
	MedicalTestPage medicalTestPage;
	List<String> transactionsLst = new ArrayList<String>();
	
	@Test
	public void DRL_TC_0049() throws Exception {
		trafficFile = dbQueries.getTrfFile_0049();
		
		commonPage = new CommonPageOnline(driver);
		commonPage.loginAsTrustedAgent("المشفى الكندي التخصصي");
		commonPage.gotoTab("Traffic File No.");
		commonPage.searchByTRFFileTrustedAgent(trafficFile);
		commonPage.gotoService("Medical Assessment History");
		commonPage.gotoService("Submit Medical Assessment Result");
		
		medicalTestPage = new MedicalTestPage(driver);
		medicalTestPage.addTest();
		
		transactionsLst.remove(transactionsLst.size() - 1);
		transactionsLst.add(commonPage.getTransactionNumber());
		
		assertEquals(commonPage.getSDDIFees(), "20");
		commonPage.payCreditCardTrustedAgeny();
	}
	
	@Test
	public void DRL_TC_0064() throws Exception {
		trafficFile = dbQueries.getTrfFile_0064();
		...
		assertEquals(commonPage.getSDDIFees(), "240");
		commonPage.payCreditCardTrustedAgeny();
	}
